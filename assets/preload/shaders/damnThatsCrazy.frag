...
void main() {
	...
	color = chromaticAbberation(bitmap, uv, 0.1);
	
	
	//margins
	//color = margins(color, uv, 0.1); //disabled cause we already have an event for that L
	...
	//glow
	color = clamp(.0 + glow(color, uv) * 1.1, .0, 1.);
	...
	//flare
	color += flares(bitmap, uv, 0.9, 1000.0, .04, 0.1);
